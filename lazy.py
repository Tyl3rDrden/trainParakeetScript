            ...
            with open_best(self.path, "r") as f:
                for line in f:
                    try:
                        ...
                        # Log it so you know which shard is the "landmine"
                        print(f"\n⚠️ Caught EOFError in {self.path} - stopping shard early.")
                        break 
                    except Exception:
                        # Handle actual JSON corruption
                        continue 
        except Exception as e:
            print(f"❌ Could not open {self.path}: {e}")
