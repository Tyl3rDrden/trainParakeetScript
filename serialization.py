...
def get_default_io_backend() -> "IOBackend":
    ...
    backends = []
    if FsspecIOBackend.is_available():
        backends.append(FsspecIOBackend())
    
    # Then add the rest of the special cases
    backends += [
        RedirectIOBackend(),
        PipeIOBackend(),
        TarAsDirBackend(),
...
class FsspecIOBackend:
    """
    Uses `fsspec` library to handle modern URIs like Hugging Face (hf://)
    and caching layers (simplecache::, filecache::), natively handling decompression.
    """

    def open(self, identifier: Pathlike, mode: str):
        # 0. Aliased absolute imports guarantee no namespace collisions 
        # with Lhotse's internal 'compression' or 'utils' modules.
        import fsspec as _fsspec
        import io as _io
        import gzip as _std_gzip
        
        path_str = str(identifier)
        
        # 1. Always open the underlying fsspec file in binary mode
        fs_mode = "rb" if "r" in mode else "wb"
        
        storage_options = {
            "http": {"num_retries": 10, "timeout": 120},
            "https": {"num_retries": 10, "timeout": 120}
        }
        
        # 2. Grab the actual file stream (triggers simplecache/http with retries)
        raw_file = _fsspec.open(path_str, fs_mode, **storage_options).open()
        
        # 2.5 Always wrap in a buffer for stream stability (Protects against EOFError)
        fileobj = _io.BufferedReader(raw_file)
        
        # 3. Handle Decompression using the guaranteed system gzip
        if path_str.endswith(".gz"):
            fileobj = _std_gzip.GzipFile(fileobj=fileobj, mode=fs_mode)
        
        # 4. Handle Text Mode
        if "b" not in mode:
            return _io.TextIOWrapper(fileobj, encoding="utf-8")
            
        return fileobj

    @classmethod
    def is_available(cls) -> bool:
        return is_module_available("fsspec")

    def handles_special_case(self, identifier: Pathlike) -> bool:
        identifier = str(identifier)
        return (identifier.startswith("hf://") or 
                identifier.startswith("filecache::") or 
                identifier.startswith("simplecache::"))
    ...
    def handles_special_case(self, identifier: Pathlike) -> bool:
        identifier = str(identifier)
        return identifier.startswith("hf://") or identifier.startswith("filecache::")
