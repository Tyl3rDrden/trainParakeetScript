...
class LazySharIterator(Dillable):
    ...
    def _init_from_dir(self, in_dir: Pathlike):
        """
        Patched to support fsspec cloud URIs (hf://, s3://, gs://, etc.)
        as well as local directories.
        """
        self.in_dir = str(in_dir)

        # 1. Resolve filesystem using fsspec
        fs, fs_path = fsspec.core.url_to_fs(self.in_dir)
        protocol = self.in_dir.split("://")[0] + "://" if "://" in self.in_dir else ""

        # 2. List all items in the directory/bucket
        raw_ls = fs.ls(fs_path)
        
        all_paths = []
        for item in raw_ls:
            # fsspec returns dicts for remote objects, strings for local
            path_str = item['name'] if isinstance(item, dict) else item
            
            # Reattach the protocol if fsspec stripped it
            if protocol and not path_str.startswith(protocol):
                full_path = protocol + path_str
            else:
                full_path = path_str
            all_paths.append(full_path)

        # 3. Helper to extract prefix (stem equivalent) without pathlib breaking on URIs
        def get_prefix(p_str: str) -> str:
            filename = p_str.split("/")[-1]
            return filename.split(".")[0]

        # 4. Discover fields and validate
        self.fields = set(get_prefix(p) for p in all_paths)
        print(f"Found the following fields in '{self.in_dir}': {self.fields}")
        assert "cuts" in self.fields, f"Could not find 'cuts' in {self.in_dir}"
        self.fields.remove("cuts")
        ...
        self.streams = {
            "cuts": sorted(
                ...
                if get_prefix(p) == "cuts" and extension_contains(".jsonl", p)
            )
        }
        for field in self.fields:
            self.streams[field] = sorted(
                p for p in all_paths if get_prefix(p) == field
            )
