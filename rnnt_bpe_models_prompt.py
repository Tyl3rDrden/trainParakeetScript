    ...
    def setup_training_data(self, train_data_config: Optional[DictConfig]):
        ...
        if 'is_tarred' in train_data_config and train_data_config['is_tarred']:
            if self._trainer is not None and isinstance(self._trainer.limit_train_batches, float):
                # Calculate a safe length: if len() fails (as it does with Lhotse streams), 
                # we catch the TypeError and provide a fallback.
                try:
                    dataset_len = len(self._train_dl.dataset)
                except (TypeError, AttributeError):
                    # Fallback: estimate based on your corpus size (e.g., 20,000)
                    # Replace 20000 with your actual approximate segment count
                    dataset_len = 20000 
                
                self._trainer.limit_train_batches = int(
                    self._trainer.limit_train_batches
                    * ceil((dataset_len / self.world_size) / train_data_config['batch_size'])
                )
